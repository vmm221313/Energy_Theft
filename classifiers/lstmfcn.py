...
class Classifier_LSTMFCN(base_Model):
	def __init__(self, output_directory, input_shape, nb_classes, verbose=False, build=True):
		super(Classifier_LSTMFCN, self).__init__()
	...
	def build_model(self, input_shape, nb_classes):
		...
		lstm = keras.layers.LSTM(8)(input_layer)
		lstm = keras.layers.Dropout(0.2)(lstm)

		permute = keras.layers.Permute((2, 1))(input_layer)
		conv1 = keras.layers.Conv1D(filters=128, kernel_size=8, padding='same')(permute)
		...
		concat = keras.layers.concatenate([lstm, gap_layer])

		output_layer = keras.layers.Dense(nb_classes, activation='softmax')(concat)
